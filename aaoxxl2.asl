...
startup {
	...
	settings.Add("startOnlyNewFile", true, "Start on new files only");
	...
	*/
	...
	vars.isLoading = null;
	vars.isLoadingPointer = null;
	
	
	vars.levelPointerST = new SigScanTarget(0,
		"1C F8 62 00 C2 04 00 8B 49 0C 8B 04 81 85 C0 75 2A" 
    );
	
	vars.isLoadingST = new SigScanTarget(0,
		"B0 29 66 00 33 C0 C2 04 00 ?? ?? ?? ?? ?? 90 90 90 90 90 90 8B 4C 24 08"
	);
...
init {
	...
	foreach (var page in game.MemoryPages(true)) {
		...
		if (ptr == IntPtr.Zero) {
			...
			IntPtr finallevelPointerPtr = new IntPtr(ptr.ToInt64());
			var finalFileOffset = (int)finallevelPointerPtr - (int)modules.First().BaseAddress;
			int[] offsets = new int[] {
				0x24
			};
			DeepPointer dP = new DeepPointer(finalFileOffset, offsets);
			var fB = dP.DerefBytes(game, 4); // would need an Array.Regverse(fB) because of Big/Little-Endian problems
			IntPtr resolvedPtr = new IntPtr();
			dP.DerefOffsets(game, out resolvedPtr);
			vars.DebugOutput("sig scan found levelPointer at " + resolvedPtr.ToString("X"));
			if(resolvedPtr != IntPtr.Zero) {
				vars.levelPointerAdress = resolvedPtr;
			}
			break;
		}
    }
	...
	foreach (var page in game.MemoryPages(true)) {
		...
		if (ptr2 == IntPtr.Zero) {
			ptr2 = scanner.Scan(vars.isLoadingST);
		} else {
			IntPtr finalIsLoading = new IntPtr(ptr2.ToInt64());
			int finalIsLoadingAddress = memory.ReadValue<int>((IntPtr) finalIsLoading);
			IntPtr finalIsLoadingAddressPointer = new IntPtr(finalIsLoadingAddress);
			vars.DebugOutput("sig scan found isLoadingPointer at " + finalIsLoadingAddressPointer.ToString("X"));
			if(finalIsLoading != IntPtr.Zero) {
				vars.isLoadingPointer = finalIsLoadingAddressPointer;
			}
			break;
		}
    }
	...
	vars.isLoading = new MemoryWatcher<int>(vars.isLoadingPointer);
			...
			if(vars.finalBossStaticPointer == null && msec >= (vars.lastFail + 100)) {
				...
				foreach (var page in game.MemoryPages(true)) {
					...
					if (ptr == IntPtr.Zero) {
						...
						DeepPointer dP = new DeepPointer(finalFileOffset, offsets);
						var fB = dP.DerefBytes(game, 1); // would need an Array.Reverse(fB) because of Big/Little-Endian problems
						IntPtr resolvedPtr = new IntPtr();
						dP.DerefOffsets(game, out resolvedPtr);
